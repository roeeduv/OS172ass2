...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    buf[i++] = digits[x % base];
     ...
     3cf:	f7 f6                	div    %esi
     3d1:	0f b6 92 50 12 00 00 	movzbl 0x1250(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
     ...
     548:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
     54b:	b8 49 12 00 00       	mov    $0x1249,%eax
...
run_thread(void (*start_func)(void *), void*arg)
{
  ...
  printf(1, "---- running the function ----\n");
     ...
     75e:	68 64 12 00 00       	push   $0x1264
     763:	6a 01                	push   $0x1
...
uthread_exit()
{
  ...
  printf(1,"exiting...\n");
     79e:	5a                   	pop    %edx
     79f:	59                   	pop    %ecx
     7a0:	68 d4 13 00 00       	push   $0x13d4
     7a5:	6a 01                	push   $0x1
...
init_bsem()
{
  ...
  if (i<MAX_BSEM){
    binSemaphore[i].id = binSemCounter;
     ...
     969:	8b 15 b4 19 00 00    	mov    0x19b4,%edx
    binSemCounter++;
  ...
  if (i<MAX_BSEM){
    ...
    binSemCounter++;
     ...
     983:	89 15 b4 19 00 00    	mov    %edx,0x19b4
    binSemaphore[i].lock = 1; // not locked
    ...
    printf(1,"all semaphores are being used\n");
     ...
     9b0:	68 84 12 00 00       	push   $0x1284
     9b5:	6a 01                	push   $0x1
...
void bsem_free(int id) {
  ...
  printf(1,"can't find id %d\n",id);
     ...
     a4b:	53                   	push   %ebx
     a4c:	68 e0 13 00 00       	push   $0x13e0
     a51:	6a 01                	push   $0x1
...
printf(1,"you tried to lock a binary semaphore that does not exist! (id: %d)\n", id);
     ...
     b33:	53                   	push   %ebx
     b34:	68 a4 12 00 00       	push   $0x12a4
     b39:	6a 01                	push   $0x1
...
void bsem_up(int id) {
  ...
  /* didnt found */
    ...
    printf(1,"you tried to free a binary semaphore that does not exist! (id: %d)\n", id);
     ...
     c63:	53                   	push   %ebx
     c64:	68 0c 13 00 00       	push   $0x130c
     c69:	6a 01                	push   $0x1
     ...
     c78:	68 e8 12 00 00       	push   $0x12e8
     c7d:	6a 01                	push   $0x1
...
uthread_init()
{
  ...
  printf(1, "-------initializing the threads -------\n");
     c96:	68 50 13 00 00       	push   $0x1350
     c9b:	6a 01                	push   $0x1
...
uthread_create(void (*start_func)(void *), void*arg)
{
  ...
  printf(1, "------ creating a new thread ------\n");
     d4f:	5b                   	pop    %ebx
     d50:	5e                   	pop    %esi
     d51:	68 7c 13 00 00       	push   $0x137c
     d56:	6a 01                	push   $0x1
  ...
  *((int*)(threads[free_thread].stack + UTHREAD_STACK_SIZE - sizeof(int))) = (int)arg;
  ...
  if(free_thread == -1){
    printf(1, "there is no available thread\n");
     ...
     dfb:	68 f2 13 00 00       	push   $0x13f2
     e00:	6a 01                	push   $0x1
...
//  printf(1, "------ scheduleing ------\n");
  ...
  for(int i = 0 ; i < MAX_UTHREADS ; i++){
    ...
    if(threads[i].state == SLEEPING_THREAD && threads[i].join!= -1 && threads[threads[i].join].state == UNUSED_THREAD) {
     ...
     f08:	68 a4 13 00 00       	push   $0x13a4
     f0d:	6a 01                	push   $0x1
...
uthread_join(int tid)
{
  ...
  /* there is a process with this tid*/
    ...
    105d:	68 2e 14 00 00       	push   $0x142e
    1062:	6a 01                	push   $0x1
  ...
  if(found_tid == -1) {
    ...
    10a3:	56                   	push   %esi
    10a4:	68 10 14 00 00       	push   $0x1410
    10a9:	6a 01                	push   $0x1
